...
*Doel: van een gecorrigeerde, gekleurde MMS-puntenwolk naar QA-gelabelde tiles die direct de BEV- en camera-preprocessor in kunnen. Optionele zware trajectstabilisatie blijft beschikbaar voor echte ruwe Riegl-exporten.*

We gaan uit van een leverancier die al GNSS/IMU-correctie, kleur en RD/NAP-coördinaten levert. Stap 1 is daarom licht en QA-gedreven, met een optionele raw-route.

---

## 1.1 Inlees- & metadata-controle
**Check:** CRS (RD New + NAP), aanwezigheid RGB/intensity, puntcount en bbox.

**Output:** `meta_scan.json` met `crs_ok`, `has_rgb`, `has_intensity`, `bbox`, `provider` om te borgen dat de dataset AHN6-achtig oogt.

---

## 1.2 Basisfiltering (lichte cleaning)
**Taken:** AOI-buffer filter, extreme Z-outliers wegfilteren, zwarte + zeer lage intensity-ruis verwijderen.
...
**Taken:** Ground filter (PMF/CSF), per punt `is_ground`, kolom `z_rel_ground`.
...
**Taken:** Clamp intensity naar [1, 255], check lichte markering vs donker asfalt, optionele simpele range-compensatie indien vendor dat niet doet.
...
**Taken:** Snijden in RD-grid tiles, meten van puntdichtheid, ground-fractie, intensity-histogram, wegvlakvulling.

**Output:** `pre_tiles/*.parquet` met `density_ok` / `density_hole`, `intensity_ok`, `ground_ok`, `crs_ok` (globaal).
...
## 1.7 Optionele raw-route (indien echte ruwe trajecten nodig zijn)
Indien je later ook onbewerkte Riegl/Applanix-exporten wilt verwerken, koppel je hier de zware trajectstabilisatie, ghost-removal en density-uniformisering terug in.

**Extra output:** `trajectory_corrected.json`, agressiever opgeschoonde LAZ, extra QA-flags zoals `trajectory_ok` en `roll_pitch_ok`.
...
- Alleen tiles met:
  ...
  - `ground_ok`
  - `crs_ok`
  - (optioneel) `basic_filter_ok` indien extra filtering is toegepast
...
## 6.1 Import van mobiele Riegl data (ruw → preproc)
We starten vanaf **pure raw MMS-output**:

- ruwe trajectory (GNSS/IMU)
- ruwe LAZ (per swath of combined)
- reflectie, intensiteit, multiple returns

De volledige **Stap 1 preprocessing** wordt hier voor het eerst end-to-end gedraaid op echte data.

**Output:**
- `corrected_points.laz`
- `trajectory_corrected.json`
- `pre_tiles/*.parquet`
- QA flags per segment & tile
